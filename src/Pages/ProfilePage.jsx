...
export default function ProfilePage() {
  ...
  const loadMailbox = async () => {
    try {
      ...
      const nextActiveList =
        activeTab === "sent" ? nextMailbox.sent : nextMailbox.inbox;
  ...
  const selectedContactStatus = useMemo(() => {
    ...
    if (friendships.friends.some((item) => String(item._id) === contactId)) {
      return "friend";
    }
    if (
      friendships.sentRequests.some((item) => String(item._id) === contactId)
    ) {
      return "sent";
    }
    if (
      friendships.receivedRequests.some((item) => String(item._id) === contactId)
    ) {
      return "received";
    }
    return "none";
  ...
  const sendFriendRequestFromMail = async () => {
    ...
    try {
      await api.post(`/api/discussion/friend-request-by-email`, {
  ...
  const acceptFriendRequestFromMail = async () => {
    if (!selectedContact?.email) return;
    try {
      await api.post(`/api/discussion/friend-request-by-email/accept`, {
        email: selectedContact.email,
      });
      await loadMailbox();
    } catch (error) {
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-6 rounded-[28px] bg-gradient-to-r from-slate-900 via-slate-800 to-cyan-900 p-6 text-white shadow-[0_24px_60px_rgba(15,23,42,0.18)]">
          ...
          <h1 className="mt-2 text-3xl font-extrabold tracking-tight">
            {user?.firstName ? `${user.firstName}'s Mail` : "Your Mail"}
          </h1>
          <p className="mt-2 text-sm text-slate-200 sm:text-base">
            Send and receive mail using your platform email address.
          </p>
                  ...
                  <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4">
                    <div>
                      <h2 className="text-lg font-bold text-slate-900">
                        {activeTab === "inbox" ? "Inbox" : "Sent Mail"}
                      </h2>
                      ...
                      <div className="border-b border-slate-200 pb-4">
                        ...
                        <div className="mt-3 space-y-1 text-sm text-slate-500">
                          ...
                          <div>
                            ...
                            {selectedMail.recipient.email}
                          </div>
                          <div>
                            <span className="font-semibold text-slate-700">Time:</span>{" "}
                            {formatMailTime(selectedMail.createdAt)}
                          </div>
                        </div>
                      </div>
                        ...
                        <div className="mt-6 rounded-[20px] border border-slate-200 bg-slate-50 p-4">
                          ...
                          <div className="mt-3 flex flex-wrap gap-2">
                            {selectedContactStatus === "none" && (
                              <button
                                onClick={sendFriendRequestFromMail}
                                className="rounded-2xl bg-cyan-600 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-cyan-500"
                              >
                                Send Friend Request
                              </button>
                            )}
                            {selectedContactStatus === "received" && (
                              <button
                                onClick={acceptFriendRequestFromMail}
                            ...
                            {selectedContactStatus === "sent" && (
                              <span className="rounded-2xl bg-amber-100 px-4 py-2.5 text-sm font-bold text-amber-700">
                                Friend Request Sent
                              </span>
                            )}
                            {selectedContactStatus === "friend" && (
                              <span className="rounded-2xl bg-emerald-100 px-4 py-2.5 text-sm font-bold text-emerald-700">
